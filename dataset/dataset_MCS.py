...
class MCSDataset(Dataset):
    ...
    def __init__(self, datapath = "dataset/newMCS", split = "train", **kargs):
        self.datapath = datapath

        super().__init__(**kargs)

        pkl_path = os.path.join(self.datapath,"mcs_data.pkl")
        data = joblib.load(open(pkl_path,'rb'))

        self._pose = [x for x in data['poses']]
        self._num_frames_in_video = [p.shape[0] for p in self._pose]
        self._joints = [x for x in data['joints_3d']]

        self.label_dict = data['label_dict']
        label_to_y = {k:v for v,k in enumerate(self.label_dict.values())}
        self._actions = [label_to_y[x] for x in data['label']]
        total_num_actions = len(self.label_dict)
